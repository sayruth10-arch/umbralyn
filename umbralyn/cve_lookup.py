...
1. Cache local (évite de re-taper l'API à chaque scan, respecte le rate-limit NVD)
    ...
    for item in data.get("vulnerabilities", []):
        ...
        for key in ("cvssMetricV31", "cvssMetricV30", "cvssMetricV2"):
            if key in metrics and metrics[key]:
                severity = (
                    metrics[key][0]["cvssData"]
                    .get("baseSeverity", "medium")
                    .lower()
                )
                break
...
def lookup_cves_for_scan(
    ...
    # Récupère la clé API une seule fois pour déterminer
    # si le rate-limit sans clé doit être appliqué.
    api_key = os.environ.get("UMBRALYN_NVD_API_KEY")
    ...
    for host in scan.hosts_up:
        for port in host.open_ports:
            ...
            if (
                cache_key in cache
                and (time.time() - cache[cache_key]["ts"]) < CACHE_TTL_SECONDS
            ):
